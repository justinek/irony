...
Furthermore, by manipulating and measuring prior expectations in a quantified manner and incorporating them in a computational model, we present a novel way of formalizing intuitions about irony to clarify the components that contribute to ironic interpretation. 
...
Other theorists have responded by invoking echoic mentions of past experiences \cite{sperber1981irony} or the notion that irony is a form of pretense \cite{clark1984pretense}.
We instead explore the idea that irony emerges from general principals of communication when properly formalized in a model that accounts for uncertain and potentially affective topics of conversation.
...
In this section, we describe the qRSA model\footnote{See \citeA{kao2014nonliteral} for modeling details and a more leisurely exposition.} and compare different spaces of affect to test the conditions for producing ironic interpretations. 
%
...
These simulations suggest that a psychologically realistic, two-dimensional affect space enables the model to interpret ironic utterances in addition to hyperbolic ones. 
...
We conducted Experiment 1 to measure prior beliefs over weather states ($P(s)$) for a range of weather contexts as well as the likelihood of various emotions towards each weather state. The latter allows us to empirically derive the affective space and priors, $P(A | s)$, for this domain.
In Experiment 2, we collected people's ratings of how a speaker perceives and feels about the weather given what she says in a weather context (e.g. ``The weather is terrible!" when the context clearly depicts sunny weather).
...
%\texttt{terrible} weather is associated with low probability of positive valence and high probability of high arousal; \texttt{amazing} weather with high probability of positive valence and high probability of high arousal.
}
\label{affect-prior}
\end{figure}
...
%, which aligns with other informal accounts of the pragmatics of nonliteral language understanding (cite).  
...
This work was supported in part by NSF Graduate Research Fellowships to JTK and by a John S. McDonnell Foundation Scholar Award and grants from the ONR to NDG.
