...
We instead explore the idea that irony will emerge from general principals of communication when properly formalized in a model that accounts for uncertain, potentially affective topics of conversation.
...
The model is capable of non-literal interpretation: it produces a hyperbolic interpretation (that the weather is merely \texttt{bad}) of the utterance in the bad weather situation. However it produces a literal interpretation (that the weather is \texttt{terrible}) in the amazing weather situation. In other words, a model that only considers valence is unlikely to infer a positive world state from a negative utterance (and vice versa), thus failing to evidence verbal irony. 
...
%(e.g. ``I was overjoyed to pay $\$30$ for an overcooked steak") 
...
%\todo{could spell out connection more?}
