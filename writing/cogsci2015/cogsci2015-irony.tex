...
We define the speaker's utility as the negative surprisal of the true state under the listener's distribution given an utterance along the QUD dimension, leading to the following utility function\footnote{See \citeA{kao2014nonliteral} for details.}:
...
\begin{figure}
\scalebox{0.45}{\includegraphics{sim12}}
...
\caption{}
\end{figure*}
\todo{JTK: should we include these figures?}
...
We selected nine images from Google Images that depict the weather (Figure~\ref{images}). To cover a range of weather states, three of the images were of sunny weather, three of cloudy weather, and three of rainy or snowy weather. We refer to these images as weather contexts.


%\begin{figure}[t]
%\scalebox{0.3}{\includegraphics{image-grid.pdf}}
%\caption{The nine weather images shown to participants.}
%\label{priors}
%\end{figure}
%
%\begin{figure}[t]
...
(TODO: add actual fit values in a clear manner: $\lambda=1$, p(state goal) = 0.2, p(valence goal) = 0.3, p(arousal goal)=0.4)
